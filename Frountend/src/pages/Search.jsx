...
useEffect(() => {
  ...
  const handleChange = (e) => {
    ...
    if (e.target.id==='offer'|| e.target.id==='parking' || e.target.id==='furnished') {
      ...
      setSearchTermData({...searchTermData, [e.target.id]: e.target.checked || e.target.checked==='true'?true:false});
    }
    if (e.target.id==='sort_order') {
      //as we provide two values togather like created_at_desc, created_at_asc
      // we need to extract seperately
      const sort = e.target.value.split("_")[0]|| /*if there is no first value then*/ 'created_at';
      // second value
      const order = e.target.value.split("_")[1]|| /*if there is no second value then*/ 'desc';
        ...
        <form  onSubmit={handleSubmit} className="flex flex-col gap-8">
          ...
          <div className="flex items-center flex-wrap gap-2 ">
            ...
            <div className="flex gap-2 ">
              ...
              onChange={handleChange}
              checked={searchTermData.type === "all"} className="w-5" />
              <span>Rent & Sele</span>
            </div>
            <div className="flex gap-2">
              <input type="checkbox" id="rent" onChange={handleChange}
              checked={searchTermData.type === "rent"} className="w-5" />
              <span>Rent</span>
            </div>
            <div className="flex gap-2">
              <input type="checkbox" id="sale" onChange={handleChange}
              checked={searchTermData.type === "sale"} className="w-5" />
              <span>Sele</span>
            </div>
            <div className="flex gap-2">
              <input type="checkbox" id="offer" onChange={handleChange}
              checked={searchTermData.offer} className="w-5" />
              <span>offer</span>
            </div>
          </div>
          ...
          <div className="flex items-center flex-wrap gap-2 ">
            ...
            <div className="flex gap-2">
              <input type="checkbox" id="parking" onChange={handleChange} checked={searchTermData.parking}   className="w-5" />
              <span>Parking</span>
            </div>
            <div className="flex gap-2">
              <input type="checkbox" id="furnished" onChange={handleChange} checked={searchTermData.furnished} className="w-5" />
              <span>Furnished</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 font-semibold" >
            ...
            onChange={handleChange} defaultValue={'created_at_desc'} className=" text-sm border border-[#147d6c] rounded-lg p-2 focus:outline-none focus:border-2">
